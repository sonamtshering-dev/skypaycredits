...
function logEvent(type, data) {
  ...
  if (isProd) {
    // In production: write structured JSON — pipe to log aggregator
...
module.exports = {
  loginFailed:     (email, ip)         => logEvent('LOGIN_FAILED',       { email, ip }),
  loginSuccess:    (userId, ip)        => logEvent('LOGIN_SUCCESS',       { userId, ip }),
  loginLocked:     (email, ip)         => logEvent('LOGIN_LOCKED',        { email, ip }),
