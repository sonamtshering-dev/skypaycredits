...
exports.register = async (req, res) => {
  try {
    ...
    try {
      ...
      await sendOTPEmail(email, otp, settings?.siteName || 'BD COINS')
    } catch (emailErr) {
...
exports.login = async (req, res) => {
  try {
    ...
    if (!user.isEmailVerified) {
      ...
      await OTP.create({ email, otp })
      const settings = await require("../models/Settings").findOne()
      await sendOTPEmail(email, otp, settings?.siteName || 'BD COINS').catch(() => {})
...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    const settings = await require('../models/Settings').findOne()
    await sendPasswordResetEmail(email, otp, settings?.siteName || 'BD COINS').catch(err => {
