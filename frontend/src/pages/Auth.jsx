...
export default function Auth() {
  ...
  useEffect(() => {
    ...
    if (window.google?.accounts?.id) { render() }
      ...
      document.getElementById('gsi-script').addEventListener('load', render, { once: true })
    }
  }, [GOOGLE_CLIENT_ID])
                ...
                <div style={{ marginTop: 16 }}>
                  ...
                  {gLoading && (
                    <div style={{ display: 'flex', justifyContent: 'center', padding: '12px 0' }}>
                      <span style={{ width: 22, height: 22, border: '2px solid rgba(255,255,255,0.3)', borderTopColor: '#fff', borderRadius: '50%', display: 'inline-block', animation: 'spin 0.7s linear infinite' }} />
                    </div>
                  )}
                  <div ref={googleBtnRef} style={{ width: '100%', opacity: gLoading ? 0 : 1 }} />
                </div>
              )}
            </>
          )}
