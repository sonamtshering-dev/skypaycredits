...
export default function AdminSupport() {
  ...
  return (
    <div style={{ display: 'flex', gap: 20, height: 'calc(100vh - 100px)', minHeight: 400 }}>

      {/* Left — ticket list */}
      <div style={{ width: 300, flexShrink: 0, display: 'flex', flexDirection: 'column', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {loading ? (
            ...
            <button key={t._id} onClick={() => openTicket(t)} style={{
              ...
              border: active?._id === t._id ? '1px solid rgba(124,58,237,0.2)' : 'none',
              borderLeft: active?._id === t._id ? '3px solid #7c3aed' : '3px solid transparent',
            }}>
      ...
      {/* Right — thread */}
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, overflow: 'hidden' }}>
        {!active ? (
          ...
          <>
            ...
            <div style={{ padding: '14px 20px', borderBottom: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
              <div style={{ flex: 1 }}>
                <div style={{ color: '#fff', fontWeight: 800, fontSize: 15 }}>{active.subject}</div>
            ...
            {active.status !== 'closed' ? (
              <div style={{ padding: '12px 20px', borderTop: '1px solid rgba(255,255,255,0.08)', display: 'flex', gap: 10, alignItems: 'flex-end' }}>
                <textarea
                  placeholder="Type your reply…"
                  value={replyText} onChange={e => setReplyText(e.target.value)}
                  rows={2}
                  onKeyDown={e => { if (e.key === 'Enter' && e.metaKey) sendReply() }}
                  style={{ flex: 1, background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 12, padding: '10px 14px', color: '#fff', fontSize: 13, outline: 'none', resize: 'vertical', fontFamily: 'inherit' }}
                />
              ...
              <div style={{ padding: '12px 20px', borderTop: '1px solid rgba(255,255,255,0.08)', textAlign: 'center', color: 'rgba(255,255,255,0.3)', fontSize: 12 }}>
